...
import { getPayment, getRazorpayConfig, processPayment, type Payment, type RazorpayConfig } from '../api/payments'
...
export function PaymentPage() {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      attempts += 1
      try {
        const payment = await getPayment(activeOrder.id)
        if (!cancelled) { setPaymentRecord(payment); setPaymentReady(true) }
      } catch {
  ...
  useEffect(() => {
    if (!paymentReady || razorpayConfig) return
    let cancelled = false
    void getRazorpayConfig().then((config) => {
      if (cancelled) return
      setRazorpayConfig(config)
      if (!config.live || window.Razorpay) { setRazorpayLoaded(true); return }
      const script = document.createElement('script')
      script.src = 'https://checkout.razorpay.com/v1/checkout.js'
      script.async = true
      script.onload = () => setRazorpayLoaded(true)
      script.onerror = () => setPaymentError('Razorpay checkout could not load. Check your connection and try again.')
      document.body.appendChild(script)
    }).catch(() => setPaymentError('Payment configuration could not be loaded. Please try again.'))
    return () => { cancelled = true }
  }, [paymentReady, razorpayConfig])

  if (!items.length || !address || !activeOrder) return (
    <main className="nc-commerce-page min-h-[70vh] bg-[var(--nc-bg)] px-5 py-12 text-[var(--nc-text)]"><div className="mx-auto max-w-3xl"><section className="rounded-[2rem] bg-slate-950 px-6 py-20 text-center text-white">
  ...
  const pay = async () => {
    if (!user || !reserved || isPaying || !paymentRecord || !razorpayConfig || !razorpayLoaded) return
    setIsPaying(true)
    setPaymentError('')
    try {
      if (razorpayConfig?.live) {
        if (!razorpayLoaded || !window.Razorpay) throw new Error('Razorpay checkout is still loading. Please try again.')
        const checkout = new window.Razorpay({ key: razorpayConfig.keyId, amount: activeOrder.totalPaise, currency: activeOrder.currency, name: 'NovaCart AI', description: activeOrder.orderNumber, order_id: paymentRecord.razorpayOrderId, prefill: { name: user.fullName, email: user.email }, theme: { color: '#dfff36' }, handler: async (response: { razorpay_payment_id: string; razorpay_signature: string }) => {
          try { const payment = await processPayment(activeOrder.id, user.id, activeOrder.totalPaise, { paymentId: response.razorpay_payment_id, signature: response.razorpay_signature }); if (payment.status !== 'CAPTURED') throw new Error(payment.failureReason || 'Payment was not completed'); setPaymentSubmitted(true) }
          catch (error) { setPaymentError(error instanceof Error ? error.message : 'Payment could not be completed. Please try again.') }
      ...
      if (!razorpayConfig?.live) setIsPaying(false)
    }
  }

  return (
    <main className="nc-commerce-page min-h-[70vh] bg-[var(--nc-bg)] px-5 py-12 text-[var(--nc-text)] sm:px-8 lg:py-16"><div className="mx-auto max-w-5xl"><CheckoutSteps current={2} />
      ...
      <div className="mt-7 grid gap-6 lg:grid-cols-[1fr_340px]">
        <section className="rounded-[2rem] border border-black/10 bg-[var(--nc-surface)] p-6 shadow-card dark:border-white/10 sm:p-8">
          <p className="nc-label">Order {activeOrder.orderNumber}</p><h1 className="mt-3 text-h2">Choose payment.</h1><p className="mt-4 text-sm leading-6 text-slate-500">Your favourites are reserved. Choose your preferred payment method to make them yours.</p>
          ...
          <div className="mt-8 rounded-[1.5rem] border border-violet-200 bg-violet-50 p-5 dark:border-[#dfff36]/20 dark:bg-[#dfff36]/[.05]">
            ...
            {reservationFailed ? <Link to="/cart" className="mt-6 flex h-12 w-full items-center justify-center rounded-full bg-slate-950 text-sm font-black text-white dark:bg-white dark:text-[#101217]">Review unavailable items</Link> : paid ? <Link to={`/orders/${activeOrder.id}`} onClick={() => { clearCart(); finishCheckout() }} className="mt-6 flex h-12 w-full items-center justify-center gap-2 rounded-full bg-emerald-600 text-sm font-black text-white"><CheckCircle2 className="h-4 w-4" /> Payment complete · Track order</Link> : <button type="button" onClick={pay} disabled={!reserved || !paymentReady || !razorpayLoaded || isPaying || paymentSubmitted} className="mt-6 flex h-12 w-full items-center justify-center gap-2 rounded-full bg-slate-950 text-sm font-black text-white transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60 dark:bg-white dark:text-[#101217]">{(reserving || (reserved && (!paymentReady || !razorpayLoaded)) || isPaying || paymentSubmitted) ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <LockKeyhole className="h-4 w-4" />}{reserving ? 'Reserving your items' : reserved && !paymentReady ? 'Preparing secure payment' : isPaying ? 'Processing payment' : paymentSubmitted ? 'Confirming your order' : `Pay securely · ₹${total.toLocaleString('en-IN')}`}</button>}
            {paymentError ? <p className="mt-3 text-center text-xs font-semibold text-red-600 dark:text-red-300">{paymentError}</p> : <p className="mt-3 text-center text-[11px] text-slate-500">Secure payment with Razorpay. Your card details stay private.</p>}
          </div>
          <div className="mt-6 flex flex-wrap gap-5 text-xs text-slate-500"><span className="flex items-center gap-2"><LockKeyhole className="h-4 w-4 text-violet-600 dark:text-[#dfff36]" /> Encrypted checkout</span><span className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-emerald-500" /> Protected payment</span></div>
        </section>
