...
export function OrdersPage() {
  ...
  return (
    <main className="nc-account-page nc-commerce-page min-h-[70vh] bg-[var(--nc-bg)] py-12 text-[var(--nc-text)] lg:py-18"><div className="nc-shell">
      <header className="flex flex-col justify-between gap-5 border-b border-black/10 pb-10 dark:border-white/10 sm:flex-row sm:items-end"><div><p className="nc-label">Purchase history</p><h1 className="mt-4 text-h1">Your orders</h1><p className="mt-4 max-w-xl text-sm leading-7 text-slate-500">From just ordered to at your doorstep. Follow your favourites here.</p></div><span className="inline-flex w-fit items-center gap-2 rounded-full border border-black/10 bg-[var(--nc-surface)] px-4 py-2 text-xs font-bold dark:border-white/10"><ReceiptText className="h-4 w-4 text-violet-600 dark:text-[#dfff36]" /> {query.isLoading ? 'Loading' : `${orders.length} ${orders.length === 1 ? 'order' : 'orders'}`}</span></header>

      {query.isLoading && <section className="mt-10 grid gap-5 lg:grid-cols-2">{[0, 1, 2, 3].map((item) => <div key={item} className="h-64 animate-pulse rounded-[1.25rem] bg-black/[.04] dark:bg-white/[.04]" />)}</section>}

      {query.isError && <section className="mt-10 rounded-[1.25rem] border border-rose-200 bg-rose-50 px-6 py-16 text-center dark:border-rose-400/20 dark:bg-rose-400/[.06]"><RefreshCw className="mx-auto h-8 w-8 text-rose-500" /><h2 className="mt-5 text-2xl font-black">We couldn’t load your orders.</h2><p className="mt-3 text-sm text-slate-500">Your history is safe. Check the connection and try again.</p><button onClick={() => query.refetch()} className="nc-primary mt-7">Try again</button></section>}

      {!query.isLoading && !query.isError && orders.length === 0 && <section className="relative mt-10 overflow-hidden rounded-[1.25rem] border border-black/10 bg-[var(--nc-surface)] px-6 py-20 text-center shadow-card dark:border-white/10 sm:px-10 sm:py-24"><div className="absolute left-[20%] top-[-8rem] h-64 w-64 rounded-full bg-violet-500/10 blur-[110px]" /><div className="relative mx-auto max-w-lg"><div className="mx-auto grid h-20 w-20 place-items-center rounded-[1.6rem] bg-slate-950 text-white"><PackageOpen className="h-8 w-8 text-[#dfff36]" /></div><p className="nc-label mt-7">No orders yet</p><h2 className="mt-4 text-h2">Your first order will appear here.</h2><p className="mx-auto mt-5 max-w-md text-sm leading-7 text-slate-500">Browse the catalogue and complete checkout to begin your NovaCart order history.</p><Link to="/categories" className="nc-primary mt-8">Explore categories <ArrowRight className="h-4 w-4" /></Link></div></section>}
