  ...
  return (
    <div className="nc-account-page min-h-screen bg-[var(--nc-bg)] py-12 text-[var(--nc-text)] lg:py-18">
      <div className="nc-shell">
        <Reveal className="grid gap-8 border-b border-white/10 pb-12 lg:grid-cols-[1fr_.55fr] lg:items-end"><div><p className="nc-label">Explore the catalogue</p><h1 className="mt-5 text-h1">A little bit of<br /><span className="nc-muted">everything you love.</span></h1></div><p className="max-w-md text-sm leading-7 text-slate-500">Explore {products.length} carefully presented products across technology, style, home, wellness, food, reading, and play.</p></Reveal>
        <div className="mt-10 grid auto-rows-[300px] gap-4 sm:grid-cols-2 lg:grid-cols-4 lg:auto-rows-[260px]">{categories.map((category, index) => <Reveal key={category.name} delay={index * .05} className={category.size}><DepthCard intensity={4} className="h-full rounded-[1.25rem]"><Link to={`/search?q=${category.name}`} className="nc-depth-surface group relative block h-full text-white overflow-hidden rounded-[1.25rem] border border-white/10 bg-ink-850"><img src={category.image} alt={`${category.name} category preview`} loading="lazy" className="h-full w-full object-cover opacity-60 transition duration-700 group-hover:scale-[1.055] group-hover:opacity-[.78]" /><div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/5 to-transparent" /><span className="nc-depth-layer absolute right-5 top-5 rounded-full border border-white/10 bg-black/25 px-3 py-1.5 text-[9px] font-bold uppercase tracking-[.14em] text-slate-300 backdrop-blur">Curated collection</span><div className="nc-depth-layer absolute inset-x-6 bottom-6 flex items-end justify-between gap-4"><div><p className="text-xs text-[#dfff36]">{category.note}</p><h2 className="mt-2 text-2xl font-semibold tracking-[-.04em]">{category.name}</h2><p className="mt-1 text-[11px] text-slate-500">{products.filter((product) => product.category === category.name).length} products</p></div><span className="grid h-11 w-11 translate-y-2 place-items-center rounded-full border border-white/15 bg-white/[.06] opacity-0 backdrop-blur transition duration-300 group-hover:translate-y-0 group-hover:opacity-100"><ArrowUpRight className="h-4 w-4" /></span></div></Link></DepthCard></Reveal>)}</div>
      </div>
    </div>
  )
}
