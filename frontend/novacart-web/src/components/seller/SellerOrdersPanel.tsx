...
import { ChevronDown, PackageCheck, Search, ShoppingBag } from 'lucide-react'
import { useMemo, useState } from 'react'
import type { Order, OrderStatus } from '../../types/saga'
...
export function SellerOrdersPanel({ orders, sellerId, loading }: { orders: Order[]; sellerId: string; loading: boolean }) {
  ...
  return <section className="mt-10"><div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><p className="nc-label">Order workspace</p><h2 className="mt-2 text-2xl font-black tracking-[-.04em]">Manage every sale.</h2><p className="mt-2 text-sm text-slate-500">Search orders and inspect only the products belonging to your store.</p></div><label className="flex h-11 min-w-64 items-center gap-2 rounded-full border border-black/10 bg-[var(--nc-surface)] px-4 dark:border-white/10"><Search className="h-4 w-4 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Order or product" className="w-full bg-transparent text-xs outline-none" /></label></div>
